	...
	Public Class SelectedCellsBorderHelper
		...
		Private Function GetSelectionBounds() As Rectangle
			...
			For Each row As GridRowInfo In info.RowsInfo
				If TypeOf row Is GridGroupRowInfo Then
					Continue For
				End If
		...
		Private Sub DrawCopyBorder(ByVal e As PaintExEventArgs)
			Dim rect As Rectangle = GetSelectionBounds()
			Paint.DrawFocusRectangle(e.Cache.Graphics, rect, Color.Black, Color.White)
		End Sub

		Private Sub DrawRegularBorder(ByVal e As PaintExEventArgs)
			Dim rect As Rectangle = GetSelectionBounds()
		...
		End Function
		Private Sub DrawDragImage(ByVal e As PaintExEventArgs, ByVal rect As Rectangle)
		...
		Private Function GetCellRect(ByVal view As GridView, ByVal rowHandle As Integer, ByVal column As GridColumn) As Rectangle
			...
			Dim cell As GridCellInfo = info.GetGridCellInfo(rowHandle, column)
